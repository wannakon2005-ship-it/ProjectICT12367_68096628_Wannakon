...
# 1. สำหรับ View v_OrderStatus
class VOrderstatus(models.Model):
    # กำหนดคอลัมน์ให้ตรงกับที่ SELECT ไว้ใน SQL View
    ...
    class Meta:
        managed = False  # สำคัญ! บอก Django ว่าไม่ต้องสร้างตารางนี้เองใน SQL
        db_table = 'v_OrderStatus' # ชื่อ View ใน SQL Server ต้องตรงเป๊ะ

# 2. สำหรับ View v_DailyRevenue
class VDailyrevenue(models.Model):
    order_date = models.DateField(db_column='Order_Date', primary_key=True)
    total_daily_revenue = models.DecimalField(db_column='Total_Daily_Revenue', max_digits=10, decimal_places=2)
